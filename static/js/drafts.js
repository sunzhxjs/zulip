...
export function restore_lastet_draft () {
    const drafts = draft_model.get();
    const narrow_drafts = filter_drafts_by_compose_box_and_recipient(drafts);
    if (Object.keys(narrow_drafts)[0] != null) {
        return {
            'draft_id': Object.keys(narrow_drafts)[0],
            'content': Object.values(narrow_drafts)[0]['content']
        }
    } else {
        return null;
    }
